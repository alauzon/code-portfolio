...
namespace Drupal\sample_simpleview_feeds\Form;

use Drupal\sample_simpleview_feeds\DateRecurSampleWidgetOptions;
use Drupal\sample_simpleview_feeds\Plugin\Field\FieldWidget\DateRecurSampleSierraWidget;
 ...
 */
class DateRecurSampleSierraModalForm extends FormBase {
  ...
  /**
   * Constructs a new DateRecurSampleSierraModalForm.
   *
  ...
  public function getFormId() {
    return 'date_recur_sample_sierra_modal';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    $form['#attached']['library'][] = 'sample_simpleview_feeds/date_recur_sample_sierra_widget_modal_form';
    $form['#attached']['library'][] = 'core/drupal.ajax';
    $form['#theme'] = 'date_recur_sample_sierra_widget_modal_form';

    $collection = $this->tempStoreFactory
      ->get(DateRecurSampleSierraWidget::COLLECTION_MODAL_STATE);

    $rrule = $collection->get(DateRecurSampleSierraWidget::COLLECTION_MODAL_STATE_KEY);
    $form['original_string'] = [
      '#type' => 'value',
      '#value' => $rrule,
    ];

    $dtStartString = $collection->get(DateRecurSampleSierraWidget::COLLECTION_MODAL_STATE_DTSTART);

    if (!empty($dtStartString)) {
      $dtStart = \DateTime::createFromFormat(DateRecurSampleSierraWidget::COLLECTION_MODAL_STATE_DTSTART_FORMAT, $dtStartString);
    }
    else {
    ...
    $form['ends']['#theme'] = 'date_recur_sample_sierra_widget_modal_form_ends';

    $endsModeDefault =
      $endsDate ? DateRecurSampleWidgetOptions::ENDS_MODE_ON_DATE :
      ($count > 0 ? DateRecurSampleWidgetOptions::ENDS_MODE_OCCURRENCES : DateRecurSampleWidgetOptions::ENDS_MODE_INFINITE);
    $form['ends']['ends_mode'] = [
      '#type' => 'radios',
      '#title' => $this->t('Ends'),
      '#options' => [
        DateRecurSampleWidgetOptions::ENDS_MODE_INFINITE => $this->t('Never'),
        DateRecurSampleWidgetOptions::ENDS_MODE_ON_DATE => $this->t('On'),
        DateRecurSampleWidgetOptions::ENDS_MODE_OCCURRENCES => $this->t('After'),
      ],
    ...
    $form['ends']['ends_count']['#states']['enabled'][] = [
      ...
      ':input[name="ends_mode"]' => ['value' => DateRecurSampleWidgetOptions::ENDS_MODE_OCCURRENCES],
    ];
    ...
    $form['ends']['ends_date']['#states']['enabled'][] = [
      // This applies correctly but Drupal has no theming for disabled dates.
      ':input[name="ends_mode"]' => ['value' => DateRecurSampleWidgetOptions::ENDS_MODE_ON_DATE],
    ];
    ...
    $form['actions']['submit'] = [
      ...
      '#ajax' => [
        ...
        'url' => Url::fromRoute('date_recur_sample_widget.sierra_modal_form'),
        'options' => [
          'query' => [
  ...
  public function ajaxSubmitForm(array &$form, FormStateInterface $form_state) {
    ...
    // Ends mode.
    if ($endsMode === DateRecurSampleWidgetOptions::ENDS_MODE_OCCURRENCES) {
      $parts['COUNT'] = (int) $form_state->getValue('ends_count');
    }
    elseif ($endsMode === DateRecurSampleWidgetOptions::ENDS_MODE_ON_DATE && $endsDate instanceof DrupalDateTime) {
    ...
    $collection = $this->tempStoreFactory
      ->get(DateRecurSampleSierraWidget::COLLECTION_MODAL_STATE);
    $collection->set(DateRecurSampleSierraWidget::COLLECTION_MODAL_STATE_KEY, implode("\n", $lines));

    $refreshBtnName = sprintf('[name="%s"]', $collection->get(DateRecurSampleSierraWidget::COLLECTION_MODAL_STATE_REFRESH_BUTTON));
    $response
