...
namespace Drupal\sample_simpleview_feeds\Plugin\Field\FieldWidget;
...
/**
 * Date recur sample widget.
 *
 * This is a widget built with Drupal states in combination with light sprinkle
 * of CSS. It is made for Sample Simpleview Feeds specific needs.
 *
 ...
 *
 * @FieldWidget(
 *   id = "date_recur_sample",
 *   label = @Translation("Sample Simpleview Feeds: Date Recur"),
 *   field_types = {
 *     "date_recur"
 *   }
 * )
 */
class DateRecurSampleWidget extends DateRecurModularWidgetBase {
  ...
  public function formElement(FieldItemListInterface $items, $delta, array $element, array &$form, FormStateInterface $form_state): array {
    /** @var DateRecurFieldItemList|\Drupal\date_recur\Plugin\Field\FieldType\DateRecurItem[] $items */
    $elementParents = array_merge($element['#field_parents'], [$this->fieldDefinition->getName(), $delta]);
    $element['#element_validate'][] = [static::class, 'validateSampleWidget'];
    $element['#after_build'][] = [static::class, 'afterBuildSampleWidget'];
    $element['#theme'] = 'date_recur_sample_widget';
    ...
    // selected mode.
    $element['ends_mode'][DateRecurSampleWidgetOptions::ENDS_MODE_OCCURRENCES]['#states'] = $this->getVisibilityStates($element, $fieldModes['ends_count'] ?? []);
    $element['ends_mode'][DateRecurSampleWidgetOptions::ENDS_MODE_ON_DATE]['#states'] = $this->getVisibilityStates($element, $fieldModes['ends_date'] ?? []);
    ...
    foreach ($fieldModes['ends_count'] ?? [] as $mode) {
      $element['ends_count']['#states']['visible'][] = [
        ...
        ':input[name="' . $nameEndsMode . '"]' => ['value' => DateRecurSampleWidgetOptions::ENDS_MODE_OCCURRENCES],
      ];
    }
    ...
    foreach ($fieldModes['ends_date'] ?? [] as $mode) {
      $element['ends_date']['#states']['visible'][] = [
        ':input[name="' . $nameMode . '"]' => ['value' => $mode],
        ':input[name="' . $nameEndsMode . '"]' => ['value' => DateRecurSampleWidgetOptions::ENDS_MODE_ON_DATE],
      ];
    }
   ...
   */
  public static function validateSampleWidget(array &$element, FormStateInterface $form_state, array &$complete_form): void {
   ...
   *   The element.
   */
  public static function afterBuildSampleWidget(array $element, FormStateInterface $form_state) {
  ...
  public function massageFormValues(array $values, array $form, FormStateInterface $form_state) {
    ...
    foreach ($values as $delta => $value) {
      ...
      // Ends mode.
      if ($endsMode === DateRecurSampleWidgetOptions::ENDS_MODE_OCCURRENCES && $mode !== static::MODE_MULTIDAY) {
